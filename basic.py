...
# at working resolution (680 pixels tall card)
workingCardHeight = 680
workingTitleCropHeight = 185
workingTitleLeftMargin = 45
workingTitleRightMargin = 1685
workingTitleHeight = 77

workingFooterCardHeight = 1360
workingFooterTopMargin = 1268
workingFooterLeftMargin = 60
workingFooterCropHeight = 60
workingFooterCropWidth = 232
...
def RemoveDuplicatesFromSorted(inList):
    outList = []
...
def GetNormalTitleSectionRect(size):
    return ScaleSectionRect((0.037, 0.045, 0.62839, 0.068), size)


def ReadTitleOfStraightCard(inImage, threshold, cardType):
    # crop to the title
    x, y, w, h = GetNormalTitleSectionRect(inImage.shape)
    ...
    size = (int(img.shape[1] * workingTitleCropHeight / img.shape[0]), workingTitleCropHeight)
    ...
    figures.sort(key = FigureAreaSort)
    figures = RemoveDuplicatesFromSorted(figures)
